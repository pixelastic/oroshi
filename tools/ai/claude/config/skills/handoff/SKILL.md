...
can continue the work. Save it to a path produced by `mktemp -t
...
Once the file is written, run `handoff-end <path>` (passing the actual file
path). This copies the launch command to the clipboard so the user can paste it
into a terminal to start a fresh Claude session with the handoff as context.
