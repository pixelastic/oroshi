...
function oroshi-prompt-populate:node_version() {
  ...
  if [[ ! -v commands[node] ]]; then
    OROSHI_PROMPT_PARTS[node_version]+="%F{$COLOR_ALIAS_ERROR}$ICONS[node] %f"
    return
  fi
  ...
  if [[ $nvmrcPath == '' ]]; then
    ...
    if [[ $nvmDefaultVersion == "$nvmCurrentVersion" ]]; then
      OROSHI_PROMPT_PARTS[node_version]+="%F{$COLOR_ALIAS_INFO}$ICONS[node] %f"
      return
    fi

    # Custom version
    OROSHI_PROMPT_PARTS[node_version]+="%F{$COLOR_ALIAS_INFO}$ICONS[node] $nvmCurrentVersion%f"
    return
  ...
  if [[ $OROSHI_NVM_LOADED == "0" ]]; then
    OROSHI_PROMPT_PARTS[node_version]+="%F{$COLOR_ALIAS_COMMENT}$ICONS[node] $expectedVersion%f"
    return
  fi

  # Local version is not even installed
  if [[ ! -d ~/.nvm/versions/node/v${expectedVersion} ]]; then
    OROSHI_PROMPT_PARTS[node_version]+="%F{$COLOR_ALIAS_ERROR}$ICONS[node] $expectedVersion%f"
    return
  fi
  ...
  if [[ $currentVersion != "$expectedVersion" ]]; then
    OROSHI_PROMPT_PARTS[node_version]+="%F{$COLOR_ALIAS_WARNING}$ICONS[node] $expectedVersion%f"
    return
  fi

  # Local version is the same as the one in use
  OROSHI_PROMPT_PARTS[node_version]+="%F{$COLOR_ALIAS_SUCCESS}$ICONS[node] $expectedVersion%f"
}
