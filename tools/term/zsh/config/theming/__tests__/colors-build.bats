...
@test "dist/colors.zsh sets YELLOW_7 ansi integer" {
  bats_run_zsh "$CURRENT"
  local script="$BATS_TMP_DIR/verify.zsh"
  cat >"$script" <<SCRIPT
source '${THEMING_ROOT}/dist/colors.zsh'
echo \${colors[YELLOW_7]}
SCRIPT
  bats_run_zsh "$script"
  [ "${lines[0]}" = "87" ]
}

@test "dist/colors.zsh sets YELLOW_7 hex string" {
  bats_run_zsh "$CURRENT"
  local script="$BATS_TMP_DIR/verify.zsh"
  cat >"$script" <<SCRIPT
source '${THEMING_ROOT}/dist/colors.zsh'
echo \${colors[YELLOW_7:hex]}
SCRIPT
  bats_run_zsh "$script"
  [ "${lines[0]}" = "#a16207" ]
}
...
@test "dist/colors.json has YELLOW_7 with correct ansi" {
  bats_run_zsh "$CURRENT"
  run jq -r '.YELLOW_7.ansi' "$THEMING_ROOT/dist/colors.json"
  [ "$output" = "87" ]
}

@test "dist/colors.json has YELLOW_7 with correct hex" {
  bats_run_zsh "$CURRENT"
  run jq -r '.YELLOW_7.hex' "$THEMING_ROOT/dist/colors.json"
  [ "$output" = "#a16207" ]
}

# --- Aliases ---

@test "aliases: colors[GIT_BRANCH] resolves to ORANGE ansi" {
  bats_run_zsh "$CURRENT"
  local script="$BATS_TMP_DIR/verify.zsh"
  cat >"$script" <<SCRIPT
source '${THEMING_ROOT}/dist/colors.zsh'
...
@test "aliases: colors[GIT_BRANCH:hex] resolves to ORANGE hex" {
  bats_run_zsh "$CURRENT"
  local script="$BATS_TMP_DIR/verify.zsh"
  cat >"$script" <<SCRIPT
source '${THEMING_ROOT}/dist/colors.zsh'
echo \${colors[GIT_BRANCH:hex]}
SCRIPT
  bats_run_zsh "$script"
  [ "${lines[0]}" = "#dd6b20" ]
}

@test "aliases: no ALIAS_GIT_BRANCH key in dist/colors.zsh" {
  bats_run_zsh "$CURRENT"
  run grep "ALIAS_GIT_BRANCH" "$THEMING_ROOT/dist/colors.zsh"
  [ "$status" -ne 0 ]
}

@test "aliases: no ALIAS_GIT_BRANCH key in dist/colors.json" {
  bats_run_zsh "$CURRENT"
  run jq 'has("ALIAS_GIT_BRANCH")' "$THEMING_ROOT/dist/colors.json"
  [ "$output" = "false" ]
}
