...
@test "--repo is passed to git-directory-root" {
  ...
  bats_run_zsh "git-dependencies-update-node abc123 --repo /some/repo --async"
  [[ "$status" -eq 0 ]]
...
@test "no commit + beacon exists → runs synchronously by default" {
  git-directory-root() { echo "$BATS_TMP_DIR/fake-repo"; }
  git-dependencies-in-progress-lockfile() { echo "/tmp/lockfile"; }
  bats_mock git-directory-root git-dependencies-in-progress-lockfile

  mkdir -p "$BATS_TMP_DIR/fake-repo"
  touch "$BATS_TMP_DIR/fake-repo/yarn.lock"

  # yarn install will fail (no package.json) but eval is still called
  bats_run_zsh "git-dependencies-update-node"
  # Exit non-zero is expected (yarn install fails in test), but eval was reached
  [[ "$output" == *"yarn"* ]] || [[ "$status" -ne 0 ]]
}
...
@test "no commit + beacon exists + --async → forks" {
  ...
  bats_run_zsh "git-dependencies-update-node --async"
  [[ "$status" -eq 0 ]]
...
@test "uses git-dependencies-in-progress-lockfile for lockfile path" {
  ...
  bats_run_zsh "git-dependencies-update-node abc123 --async"
  [[ "$status" -eq 0 ]]
