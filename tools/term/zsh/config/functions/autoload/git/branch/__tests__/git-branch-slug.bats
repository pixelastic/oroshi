...
@test "replaces multiple slashes with underscores" {
  bats_run_zsh "git-branch-slug 'feat/scope/deep'"
  [ "$status" -eq 0 ]
  [ "$output" = "feat_scope_deep" ]
}

@test "leaves plain branch name unchanged" {
  bats_run_zsh "git-branch-slug 'main'"
  [ "$status" -eq 0 ]
  [ "$output" = "main" ]
}

@test "preserves hyphens and underscores" {
  bats_run_zsh "git-branch-slug 'fix/my-bug_test'"
  [ "$status" -eq 0 ]
  [ "$output" = "fix_my-bug_test" ]
}

@test "replaces dots with hyphens" {
  bats_run_zsh "git-branch-slug 'progress.json'"
  [ "$status" -eq 0 ]
  [ "$output" = "progress-json" ]
}

@test "replaces slashes and dots in combination" {
  bats_run_zsh "git-branch-slug 'feat/fix.thing'"
  [ "$status" -eq 0 ]
