...
setup() {
  bats_tmp_dir
  CURRENT="$BATS_TEST_DIRNAME/../simplify-path"
}
...
@test "maxDisplay=3 silently falls back to 4" {
  bats_run_zsh "$CURRENT" "a/b/c/d/e" 3
  [ "$output" = "a/…/d/e" ]
}
...
@test "--reply does not echo" {
  bats_run_zsh "$CURRENT" --reply "a/b/c/d/e"
  [ "$output" = "" ]
}
