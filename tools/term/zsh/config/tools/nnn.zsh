...
function oroshi_nnn_colors() {
	...
	local NNN_EXECUTABLE="${(l:2::0:)COLORS[executable]}"
	local NNN_REGULAR="00"
	local NNN_HARD_LINK="00"
	local NNN_SYMBOLIC_LINK="${(l:2::0:)COLORS[link]}"
	local NNN_MISSING_OR_FILE_DETAILS="${(l:2::0:)COLORS[error]}"
	local NNN_ORPHANED_SYMBOLIC_LINK="${(l:2::0:)COLORS[error]}"
	local NNN_FIFO="00"
	local NNN_SOCKET="00"
