...
## Session 2026-05-27 — 0017: old directory trees deleted and no stale references remain

- Completed: Migrated 11 unmigrated domains (download, security, video, vms, compilers, pdf, apis, gaming, ebook, disk, ttrpg) — 47 install scripts git mv'd to tools/{domain}/{tool}/install. Converted video/index and compilers/index to install-all (zsh shebang, ${0:h} paths). Migrated scripts/deploy/sysctl to tools/infrastructure/sysctl/deploy (updated to zsh + set -e + ${0:h}/config). Moved config/__archive/sysctl/sysctl.conf to tools/infrastructure/sysctl/config/. Deleted config/__archive/alacritty and config/__archive/colors (archive, not migrated). Removed empty scripts/install/, scripts/deploy/, config/ dirs. Fixed stale ~/.oroshi/config/ refs in: tools/term/kitty/config/kitty.conf (7 includes), keybindings.conf (scrollback_pager), scrollback_pager (source path), tab_bar_modules/projects.py (json path), tools/cli/bat/config/batrc.conf (map-syntax glob), tools/ai/claude/config/skills/zsh-writer/references/calling-commands.md, skill-writer/references/skill-template.md, tools/keybindings/xbindkeys/config/xbindkeysrc.conf (comment), tools/_languages/markdown/textlint/config/textlintrc.js (terms.json path), scripts/bin/spotify/spotify, scripts/bin/remark-lint, scripts/bin/remark-fix, scripts/bin/colors-refresh (3 refs), scripts/bin/git/directory/git-directory-create, scripts/bin/audio/audio-play-oroshi, scripts/bin/kitty/kitty-debug, scripts/bin/kitty/kitty-restore (commented), scripts/bin/watch-and-reload, scripts/bin/term/bats/__tests__/bats-test-path.bats (4 refs). Fixed stale ~/.oroshi/scripts/deploy/ refs in tools/term/zsh/config/functions/autoload/misc/ymdk-sync and ymdk-build. Fixed local at script scope in steam/shadow/algolia/anyflip-downloader/calibre install scripts. Fixed googleworkspace cross-tool ref (OROSHI_PATH → OROSHI_ROOT/tools/). Fixed gta echo (wine config path).
- Tests added: none (migration/cleanup issue — no automated tests per progress.md guidance)
- Discovered: 11 additional domains were not in the original 16 PRD issues. remark/textlint configs were already in tools/_languages/markdown/ from a prior cleanup commit (ea357f47). sysctl config was in __archive but still referenced by scripts/deploy/sysctl — rescued to tools/infrastructure/sysctl/config/. kitty.conf had 7 include paths needing update, all with ~/.oroshi/config/ prefix. Several scripts/bin/ files had stale cross-tool refs missed in domain migrations (colors-refresh, git-directory-create, audio-play-oroshi, spotify, watch-and-reload, bats tests). scripts/bin/term/bats/bats wrapper used old config/term/zsh/ paths — breaking all bats tests in the worktree because bats-test-path subprocesses sourced ~/.zshenv with OROSHI_ROOT pointing to the worktree where config/term/zsh/ no longer exists.
- Fixed: per review — kitty-debug $HOME → $OROSHI_ROOT; sysctl/deploy bash→zsh shebang + set -e; calibre/install missing set -e. Post-vca: bats wrapper updated for new path structure + ZDOTDIR override so subprocess zsh processes use worktree zshenv during transition; bats-test-path.bats uses $OROSHI_ROOT instead of $HOME/.oroshi.
- Skipped feedback: local in scripts/bin/ (pre-existing, do-not-touch files); gta/install bash shebang (echo-only, harmless); remark-fix/remark-lint use ~/.oroshi instead of $OROSHI_ROOT (bin files, do-not-touch constraint, functionally correct).
- Next: all issues complete — reorg branch ready for commit and merge. After merge: update ~/.zshenv symlink to point to tools/term/zsh/config/zshenv.zsh (run tools/term/zsh/deploy)
