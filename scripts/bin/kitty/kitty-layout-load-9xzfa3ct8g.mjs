...
const KittyLoad = {
  ...
  populateManifest(tabs) {
    _.each(tabs, (tab) => {
      const tabId = tab.id;
      const firstWindowId = this.getMainSplitWindowId(tab.layout_state.pairs);
  ...
  addSplitToScript(layoutState) {
    ...
    const mainWindow = manifest[this.getMainSplitWindow(layoutState.one)];
    this.kitty('focus-window', {
      match: `var:OROSHI_RESTORE_UUID=${mainWindow.uuid}`,
    });

    // Split the window
    const secondaryWindow = manifest[this.getMainSplitWindow(layoutState.two)];
