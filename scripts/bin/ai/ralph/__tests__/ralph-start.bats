...
@test "outputs status:ready with first eligible issue" {
  printf '[{"id":"01","issue":"issues/01-foo.md","done":false,"blocked_by":[]}]' > "$PLAN_DIR/state.json"
...
@test "picks lowest id when multiple eligible" {
  printf '[{"id":"01","issue":"issues/01-foo.md","done":true,"blocked_by":[]},{"id":"02","issue":"issues/02-bar.md","done":false,"blocked_by":[]},{"id":"03","issue":"issues/03-baz.md","done":false,"blocked_by":[]}]' > "$PLAN_DIR/state.json"
  bats_run_script "$RALPH_START" "$PLAN_DIR"
  [ "$status" -eq 0 ]
...
@test "skips issues with unresolved blockers" {
  printf '[{"id":"01","issue":"issues/01-foo.md","done":false,"blocked_by":[]},{"id":"02","issue":"issues/02-bar.md","done":false,"blocked_by":["01"]}]' > "$PLAN_DIR/state.json"
  bats_run_script "$RALPH_START" "$PLAN_DIR"
  [ "$status" -eq 0 ]
  [ "$(echo "$output" | jq -r '.id')" = "01" ]
}
...
@test "outputs status:finished when all issues complete" {
  printf '[{"id":"01","issue":"issues/01-foo.md","done":true,"blocked_by":[]},{"id":"02","issue":"issues/02-bar.md","done":true,"blocked_by":[]}]' > "$PLAN_DIR/state.json"
  bats_run_script "$RALPH_START" "$PLAN_DIR"
  [ "$status" -eq 0 ]
...
@test "outputs status:deadlocked when all issues are blocked" {
  printf '[{"id":"01","issue":"issues/01-foo.md","done":false,"blocked_by":["02"]},{"id":"02","issue":"issues/02-bar.md","done":false,"blocked_by":["01"]}]' > "$PLAN_DIR/state.json"
  bats_run_script "$RALPH_START" "$PLAN_DIR"
  [ "$status" -eq 0 ]
...
@test "picks lowest id even when state.json is out of order" {
  printf '[{"id":"03","issue":"issues/03-baz.md","done":false,"blocked_by":[]},{"id":"01","issue":"issues/01-foo.md","done":false,"blocked_by":[]},{"id":"02","issue":"issues/02-bar.md","done":false,"blocked_by":[]}]' > "$PLAN_DIR/state.json"
  bats_run_script "$RALPH_START" "$PLAN_DIR"
  [ "$status" -eq 0 ]
  [ "$(echo "$output" | jq -r '.id')" = "01" ]
}
