...
@test "outputs done▮total for mixed passes values" {
  printf '[{"passes":true},{"passes":false},{"passes":false}]' > "$PRD_DIR/prd.json"
...
@test "counts only passes==true strictly (null and string excluded)" {
  printf '[{"passes":true},{"passes":false},{"passes":null},{"passes":"true"}]' > "$PRD_DIR/prd.json"
  bats_run_script "$RALPH_PROGRESS" "$PRD_DIR"
  [ "$status" -eq 0 ]
  [ "$output" = "1▮4" ]
}
...
@test "outputs done▮total with all passes true" {
  printf '[{"passes":true},{"passes":true}]' > "$PRD_DIR/prd.json"
  bats_run_script "$RALPH_PROGRESS" "$PRD_DIR"
  [ "$status" -eq 0 ]
  [ "$output" = "2▮2" ]
}
...
@test "outputs done▮total with all passes false" {
  printf '[{"passes":false},{"passes":false}]' > "$PRD_DIR/prd.json"
  bats_run_script "$RALPH_PROGRESS" "$PRD_DIR"
  [ "$status" -eq 0 ]
  [ "$output" = "0▮2" ]
}
...
@test "exits 1 for empty array" {
  printf '[]' > "$PRD_DIR/prd.json"
  bats_run_script "$RALPH_PROGRESS" "$PRD_DIR"
  [ "$status" -eq 1 ]
}

@test "exits 1 for malformed JSON" {
  printf 'not valid json' > "$PRD_DIR/prd.json"
  bats_run_script "$RALPH_PROGRESS" "$PRD_DIR"
  [ "$status" -eq 1 ]
}
...
@test "deduces directory from worktree context when called with no argument" {
  ...
  mkdir -p "$wt_path/docs/feat_no-arg"
  printf '[{"passes":true},{"passes":false}]' > "$wt_path/docs/feat_no-arg/prd.json"
  cd "$wt_path"
