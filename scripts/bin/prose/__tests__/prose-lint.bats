...
@test "stdin input: produces same output format as file input" {
  vale() {
    printf '{"stdin.txt":[{"Action":{"Name":"","Params":null},"Span":[9,12],"Check":"proselint.Very","Description":"","Link":"","Message":"Remove very.","Severity":"error","Match":"very","Line":2}]}\n'
  }
  bats_mock vale

  bats_run_zsh "printf 'This is very good.\n' | prose-lint"
  [[ "$status" -eq 1 ]]
  local keys="$(printf '%s' "$output" | jq -r '.[0] | keys | sort | join(",")')"
  [[ "$keys" == "line,match,message,rule,severity" ]]
}
