...
- Sidequests shouldn't save their files in /tmp, it gets deleted on reboot and I
  could lose context if I didn't get to a /plan
- meetup-announce should use bulletpoints (in #team-devmarketing) for
scannability
- Tell claue to run log analysis in subagents
- Show in Claude statusbar the number of subagents running
- theme.go in git-branch-pretty, git-file-watch and git-commit-diffstat is
similar and might need to be extracted in a shared helper
