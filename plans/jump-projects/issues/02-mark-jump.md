...
2. Check `$MARKPATH/$1` — if exists, cd via symlink (`cd -P`)
...
- given a symlink $MARKPATH/bar pointing to /tmp/target and no project named bar
- mark-jump bar cd's to /tmp/target

**mark-jump prefers project over mark on collision:**
- given both PROJECTS[baz:path] and $MARKPATH/baz exist
